...
class _CustomBottomDrawerState extends State<CustomBottomDrawer>
  ...
  void _buildVerticalDragGestureRecognizerInitializer(
      VerticalDragGestureRecognizer gestureRecognizer) {
    gestureRecognizer
      ..onStart = onDragStart
    ...
    if (widget.followTheBody)
      switch (notification.runtimeType) {
        ...
        case OverscrollNotification:
          ...
          double pixels = metrics.pixels;
          double flag = pixels - scrollOffset;
          if (scrollOffset == 0.0 && flag == 0.0 && scrollAtEdge && opened)
            close();
          return true;
      }
    return false;
  ...
  void close([bool? callback]) {
    //print(widget.pageController.page!.toInt());
    //print(widget.scrollControllers[widget.pageController.page!.toInt()].offset);
    if (widget.scrollControllers[widget.pageController.page!.toInt()].offset ==
        widget.scrollControllers[widget.pageController.page!.toInt()]
            .initialScrollOffset) {
      if (opened) {
